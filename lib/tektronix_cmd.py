...
class DPO4000():
    ...
    def setup(self):
        ...
        try:
            self.timeout = config_json_data["Timeout"]
        except Exception as e:
            print(e)
            pass
    ...
    def get_raw_bin(self):
        try:
            time.sleep(self.cmd_timeout)
            bin_wave = self.scope.query_binary_values('CURVE?', datatype='b', container=np.array, is_big_endian=True)
            print("(raw)        : Execute the Command Successfully : %s" %("CURVE?"))
            return bin_wave
        except Exception as e: 
            print(e)
